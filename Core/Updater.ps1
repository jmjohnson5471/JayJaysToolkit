function Invoke-JJTUpdate {
    ...
    try {
        ...
        if (Test-Path $backup) { Copy-Item "$backup\*" $Script:BaseDir -Recurse -Force -ErrorAction SilentlyContinue }
        [System.Windows.Forms.MessageBox]::Show("Update failed. Rollback was attempted.`n$($_.Exception.Message)","JayJaysToolkit") | Out-Null
    }
}
